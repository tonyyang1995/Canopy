...
class RidgeRegressionModel():
    def __init__(self, configs):
        ...
        start = configs["Model"]["start"]
        stop = configs["Model"]["stop"]
        num = configs["Model"]["num"]
    ...
    def load(self):
        if not os.path.exists(f"{self.save_path}/ridge_regression.pkl"):
            print("Model checkpoint not found!")
            return
        
        with open(f"{self.save_path}/ridge_regression.pkl", "rb") as f:
